...
class Admin extends Controller{
    ...
    public function editNewNew($id){
        $nn = $this->model->getTheNewById($id);
        if($nn != false)
            $this->view->new = $nn[0];
        // echo var_dump($nn);
        if(
            ...
            isset($_POST['n_title_en']) && !empty(trim($_POST['n_title_en'])) &&
            isset($_POST['n_title_tr']) && !empty(trim($_POST['n_title_tr'])) &&
            isset($_POST['n_desc_ar']) && !empty(trim($_POST['n_desc_ar'])) &&
            isset($_POST['n_desc_en']) && !empty(trim($_POST['n_desc_en'])) &&
            isset($_POST['n_desc_tr']) && !empty(trim($_POST['n_desc_tr']))
        ){
            $ar_url = str_ireplace(" ", "-", $_POST['n_title_ar']);
            $en_url = str_ireplace(" ", "-", $_POST['n_title_en']);
                
            $info = [
                'n_id' => $id,
                'n_user' => intval($_SESSION['user_id']),
                'n_date' => $this->protect($_POST['n_date']),
                'n_img' => $this->protect($_POST['n_img']),
                'n_section' => intval($_POST['n_section']),
                'n_title_ar' => $this->protect($_POST['n_title_ar']),
                'n_title_en' => $this->protect($_POST['n_title_en']),
                'n_title_tr' => $this->protect($_POST['n_title_tr']),
                'n_desc_ar' => $this->protect($_POST['n_desc_ar']),
                'n_desc_en' => $this->protect($_POST['n_desc_en']),
                'n_desc_tr' => $this->protect($_POST['n_desc_tr']),
                'n_title_url_ar' => $ar_url,
                'n_title_url_en' => $en_url,
                'n_title_url_tr' => $en_url
            ];

            $enn = $this->model->editNewNew($info);
            if($enn == 'ex')
                echo "try another title, title exists";
            elseif($enn == 'good')
                $this->redirect(ADMIN_PATH . "/new-new?edit_new=" . $id);
            else
                echo "err153649";
            echo var_dump($enn);
        }
    ...
    public function newNew(){
        if($this->checkUSession() == false){  // if not logged in
            ...
            $news_sections = $this->model->getAllNewsSections();
            $this->view->nsec = [];
            if($news_sections != false)
                $this->view->nsec = $news_sections;

            if(isset($_GET['del_new']) && !empty(trim($_GET['del_new']))){
                $del_id = intval($_GET['del_new']);
                $this->model->deleteTheNew($del_id);
                $this->redirect(ADMIN_PATH . "/new-new?all_news=1");
            }

            if(isset($_GET['edit_new']) && !empty(trim($_GET['edit_new']))){
                $edit_id = intval($_GET['edit_new']);
                $this->editNewNew($edit_id);
            }
            
            if(             
                !isset($_GET['edit_new']) && !empty(trim($_GET['edit_new'])) &&
                isset($_POST['n_date']) && !empty(trim($_POST['n_date'])) &&
                isset($_POST['n_img']) && !empty(trim($_POST['n_img'])) &&
                isset($_POST['n_section']) && !empty(trim($_POST['n_section'])) &&
                isset($_POST['n_title_ar']) && !empty(trim($_POST['n_title_ar'])) &&
                ...
                $cnn = $this->model->createNewNew($info);
                if($cnn == 'good'){
                    echo "created";
                }elseif($cnn == 'ex'){
                    echo "Please try another titles";
                }
            }
            if(isset($_GET['all_news']) && $_GET['all_news'] === "1"){
                $this->view->news = [];
                $news = $this->model->getAllNews();
                if($news != false)
                    $this->view->news = $news;
                $this->view->view('admin/new-new-all');
            }
            else{
                if(isset($_GET['edit_new']) && !empty(trim($_GET['edit_new'])))
                    $this->view->view('admin/edit-new');
                else
                    $this->view->view('admin/new-new');
            }
        }
    }
    
    public function editNSec($id){
        $the_sec = $this->model->getNSecById($id);
        if($the_sec == false){
            $this->redirect(ADMIN_PATH . '/new-nsec');
        }else{
            $this->view->the_sec = $the_sec[0];
            if(isset($_POST['ns_name']) && !empty(trim($_POST['ns_name'])) &&
               isset($_POST['ns_name_en']) && !empty(trim($_POST['ns_name_en'])) &&
               isset($_POST['ns_name_tr']) && !empty(trim($_POST['ns_name_tr']))){

                $info = [
                    'id' => $id,
                    'ns_name' => $this->protect($_POST['ns_name']),
                    'ns_name_en' => $this->protect($_POST['ns_name_en']),
                    'ns_name_tr' => $this->protect($_POST['ns_name_tr']),
                ];

                $edit_nsec = $this->model->editNSec($info);
                if($edit_nsec != false){
                    echo "edited";
                    echo "<script>setTimeout(function(){window.location.href = ''},1200)</script>";
                }
                // echo var_dump($edit_nsec);
            }

            $this->view->view('admin/edit-nsec');
        }
    }

    // create new section for news
    public function newNSec(){
        if($this->checkUSession() == false){  // if not logged in
            $this->redirect(ADMIN_PATH . '/login');
        }else{
            $all_news_section = $this->model->getAllNewsSections();
            $this->view->allNSections = [];
            if($all_news_section != false)
                $this->view->allNSections = $all_news_section;

            if(isset($_GET['del_id']) && !empty(trim($_GET['del_id']))){
                $del_id = intval($_GET['del_id']);
                echo var_dump($this->model->delNSec($del_id));
                $this->redirect(ADMIN_PATH . "/new-nsec");
            }elseif(isset($_GET['edit_id']) && !empty(trim($_GET['edit_id']))){
                $edit_id = intval($_GET['edit_id']);
                $this->editNSec($edit_id);
            }else{
                if(isset($_POST['ns_name']) && !empty(trim($_POST['ns_name'])) &&
                   isset($_POST['ns_name_en']) && !empty(trim($_POST['ns_name_en'])) &&
                   isset($_POST['ns_name_tr']) && !empty(trim($_POST['ns_name_tr']))){
                    ...
                    $new_nsec = $this->model->createNSec($info);
                    if($new_nsec === true){
                        echo "Created";
                        echo "<script>setTimeout(function(){window.location.href = ''},1200)</script>";
                    }
                    // echo var_dump($new_nsec);
                }
        
                $this->view->view('admin/new-nsec');
            }
        }
    }
